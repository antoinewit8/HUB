...
if st.session_state.get("uploaded_bytes") and st.button("🚀 Lancer le calcul", type="primary"):
    try:
        ...
        with st.status("⚙️ Calcul en cours...", expanded=True) as status_widget:
            ...
            if result["success"]:
                status_widget.update(label="✅ Calcul terminé !", state="complete")
            else:
                status_widget.update(label="❌ Erreur durant le calcul", state="error")
